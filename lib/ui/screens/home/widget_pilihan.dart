import 'package:flutter/material.dart';
import 'package:flutter_staggered_grid_view/flutter_staggered_grid_view.dart';
import 'package:mbakurir/common/my_colors.dart';
import 'package:mbakurir/common/my_font_size.dart';
import 'package:mbakurir/ui/screens/tracking/tracking_judulatas.dart';
import 'package:mbakurir/ui/screens/zbawa/atas.dart';
import 'package:mbakurir/ui/widgets/custom_card.dart';

class WidgetStraggeredGridView extends StatefulWidget {
  const WidgetStraggeredGridView({Key? key}) : super(key: key);

  @override
  _WidgetStraggeredGridViewState createState() => _WidgetStraggeredGridViewState();
}

class _WidgetStraggeredGridViewState extends State<WidgetStraggeredGridView> {
  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.all(20),
      child: StaggeredGrid.count(
        crossAxisCount: 2,
        mainAxisSpacing: 20,
        crossAxisSpacing: 20,
        children: [
          CustomCard(
            onTap: () {
              Navigator.push(context, MaterialPageRoute(builder: (context) => TrackingShipment()));
            },
            shadow: false,
            width: double.infinity,
            bgColor: MyColors.blue,
            borderRadius: BorderRadius.circular(15),
            padding: EdgeInsets.all(18),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                CustomCard(
                  shadow: false,
                  height: 50,
                  width: 50,
                  bgColor: MyColors.purple,
                  borderRadius: BorderRadius.circular(100),
                  child: Center(
                    child: Icon(Icons.calculate),
                  )
                ),
                SizedBox(height: 15,),
                Text(
                  "Hitung jumlah pengiriman",
                  style: TextStyle(
                    color: MyColors.white,
                    fontSize: MyFontSize.medium1,
                    fontWeight: FontWeight.bold
                  ),
                ),
                SizedBox(height: 10,),
                Text(
                  "Semua opsi harga didasarkan pada harga yang ada",
                  style: TextStyle(
                    color: MyColors.white.withOpacity(.8),
                    fontSize: MyFontSize.small3,
                  ),
                ),
              ],
            ),
          ),
          CustomCard(
            onTap: () {
              Navigator.push(context, MaterialPageRoute(builder: (context) => TrackingAtas()));
            },
            shadow: false,
            width: double.infinity,
            bgColor: MyColors.softGrey,
            borderRadius: BorderRadius.circular(15),
            padding: EdgeInsets.all(18),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                CustomCard(
                  shadow: false,
                  height: 50,
                  width: 50,
                  bgColor: MyColors.purple,
                  borderRadius: BorderRadius.circular(100),
                  child: Center(
                    child: Icon(Icons.car_rental)
                  )
                ),
                SizedBox(height: 15,),
                Text(
                  "Lacak Lokasi Pengiriman Anda",
                  style: TextStyle(
                    color: MyColors.blackText,
                    fontSize: MyFontSize.medium1,
                    fontWeight: FontWeight.bold
                  ),
                ),
                SizedBox(height: 10,),
                Text(
                  "Kontrol paket Anda seperti Boss, Anda akan tahu setiap pergerakan paket dan perkiraannya",
                  style: TextStyle(
                    color: MyColors.blackText.withOpacity(.8),
                    fontSize: MyFontSize.small3,
                  ),
                ),
              ],
            ),
          ),
          CustomCard(
            shadow: false,
            width: double.infinity,
            bgColor: MyColors.softGrey,
            borderRadius: BorderRadius.circular(15),
            padding: EdgeInsets.all(18),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                CustomCard(
                  shadow: false,
                  height: 50,
                  width: 50,
                  bgColor: MyColors.purple,
                  borderRadius: BorderRadius.circular(100),
                  child: Center(
                    child: Icon(Icons.analytics)
                  )
                ),
                SizedBox(height: 15,),
                Text(
                  "Riwayat pesanan Anda",
                  style: TextStyle(
                    color: MyColors.blackText,
                    fontSize: MyFontSize.medium1,
                    fontWeight: FontWeight.bold
                  ),
                ),
                SizedBox(height: 10,),
                Text(
                  "Lihat semua riwayat pesanan Anda. Lihat bagaimana kami mengirimnya begitu cepat",
                  style: TextStyle(
                    color: MyColors.blackText.withOpacity(.8),
                    fontSize: MyFontSize.small3,
                  ),
                ),
              ],
            ),
          ),
          CustomCard(
            shadow: false,
            width: double.infinity,
            bgColor: MyColors.softGrey,
            borderRadius: BorderRadius.circular(15),
            padding: EdgeInsets.all(18),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                CustomCard(
                  shadow: false,
                  height: 50,
                  width: 50,
                  bgColor: MyColors.purple,
                  borderRadius: BorderRadius.circular(100),
                  child: Center(
                    child: Icon(Icons.restore)
                  )
                ),
                SizedBox(height: 15,),
                Text(
                  "Kembalikan barang yang rusak",
                  style: TextStyle(
                    color: MyColors.blackText,
                    fontSize: MyFontSize.medium1,
                    fontWeight: FontWeight.bold
                  ),
                ),
                SizedBox(height: 10,),
                Text(
                  "Kami menerima pengembalian barang rusak karena pengiriman kami yang kurang optimal, atau campur tangan pihak luar yang menyebabkan barang Anda rusak. Kami tahu bahwa kepuasan pelanggan adalah kepuasan kami",
                  style: TextStyle(
                    color: MyColors.blackText.withOpacity(.8),
                    fontSize: MyFontSize.small3,
                  ),
                ),
              ],
            ),
          ),
        ],
      ),
    );
  }
}